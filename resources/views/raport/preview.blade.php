...
<head>
    ...
    <title>Rapor Santri - {{ optional($record->student)->name }}</title>
...
<x-filament::modal>
    ...
    <div class="report-container">
        ...
        <div class="report-header">
            <div> <img width="100%" src="/storage/kop.jpg" alt="" srcset=""></div>
                ...
                <div class="info-left">
                    <table>
                        <tr>
                            <td>Nama Santri</td>
                            <td>&nbsp;:&nbsp;</td>
                            <td class="font-bold">{{ optional($record->student)->name }}</td>
                        </tr>
                        <tr>
                            <td>Orang Tua/Wali</td>
                            <td>&nbsp;:&nbsp;</td>
                            <td>{{ optional($record->student)->guardian_name ?? '-' }}</td>
                        </tr>
                    </table>
                </div>
                <div class="info-right">
                    <table>
                        <tr>
                            <td>Kelas</td>
                            <td>&nbsp;:&nbsp;</td>
                            <td>{{ optional($record->classroom)->name }}</td>
                        </tr>
                        <tr>
                            <td>Tahun Pelajaran</td>
                            <td>&nbsp;:&nbsp;</td>
                            <td>{{ optional($record->schoolYear)->name }}</td>
                        </tr>
                    </table>
                </div>
            </div>
        ...
        <table class="report-card">
            ...
            <tbody>
                <tr class="header-row">
                    ...
                    <td class="vertical-divider" rowspan="{{ max(($record->values ?? collect())->count(), 13) + 4 }}">
                    </td>
                ...
                @php
                    ...
                    $sortedValues = ($record->values ?? collect())->sortBy(function ($value) {
                ...
                @for ($i = ($record->values ?? collect())->count(); $i < 13; $i++)
                ...
                <tr class="summary-row">
                    ...
                    <td class="text-center">{{ toEasternArabicNumerals($record->total_score) }}</td>
                ...
                <tr class="summary-row">
                    ...
                    <td class="font-italic"></td>
                    <td class="text-right" colspan="2"></td>
                    <td class="text-center">{{ toEasternArabicNumerals($record->rank) }}</td>
                ...
                <tr class="personality-row">
                    ...
                    <td colspan="1">:</td>
                    <td class="text-center">{{ $record->behavior }}</td>

                    {{-- ABSENSI (3 columns) --}}
                    <td>Sakit</td>
                    <td>:</td>
                    <td class="text-center">{{ $record->presence_sick ?? 0 }}</td>

                    {{-- كشف الغياب (6 columns) --}}
                    <td class="text-center">{{ toEasternArabicNumerals($record->presence_sick ?? 0) }}</td>
                    <td>:</td>
                ...
                <tr class="personality-row">
                    ...
                    <td colspan="1">:</td>
                    <td class="text-center">{{ $record->orderly }}</td>

                    {{-- ABSENSI (3 columns) --}}
                    <td colspan="1">Izin</td>
                    <td>:</td>
                    <td class="text-center">{{ $record->presence_permission ?? 0 }}</td>

                    {{-- كشف الغياب (6 columns) --}}
                    <td class="text-center">{{ toEasternArabicNumerals($record->presence_permission ?? 0) }}</td>
                    <td>:</td>
                ...
                <tr class="personality-row">
                    ...
                    <td colspan="1">:</td>
                    <td class="text-center">{{ $record->perseverance }}</td>

                    {{-- ABSENSI (3 columns) --}}
                    <td>Alpa</td>
                    <td>:</td>
                    <td class="text-center">{{ $record->presence_absen ?? 0 }}</td>

                    {{-- كشف الغياب (6 columns) --}}
                    <td class="text-center">{{ toEasternArabicNumerals($record->presence_absen ?? 0) }}</td>
                    <td>:</td>
                ...
                <tr class="notes-content">
                    <td colspan="15">{{ $record->guardian_note }}</td>
                </tr>
                <tr class="notes-header">
                    <td colspan="15">Catatan Kepala Madrasah</td>
                </tr>
                <tr class="notes-content">
                    <td colspan="15">{{ $record->head_note }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="signature-section">
            ...
            <div class="signature-box">
                <p>Bojonegoro, {{ \Carbon\Carbon::parse($record->print_date)->translatedFormat('d F Y') }}</p>
                <p>Wali Kelas</p>
                <p class="name">({{ optional(optional($record->classroom)->teacher)->name ?? 'Nama Wali Kelas' }})
                </p>
            </div>
            <div class="signature-box">
                <p>&nbsp;</p>
